...
class MatchResult extends React.Component {
	...
	componentDidMount() {
        fetch(window.backendHost + '/api/matches/search/findByRound?round=' + this.state.actualRound)
	...
	render() {
        var matchNodes = this.state.matches.map(function (match, i) {
			...
			var homeStyle = {color: 'white'}, awayStyle = {color: 'white'}, wonStyle = {color: 'white', fontWeight: 'bold'};
